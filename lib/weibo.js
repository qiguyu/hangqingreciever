...
exports.getLatest = function(param, cb) {
//    mysql._select('sent_micro_blog', 1, param.cnt, 'micro_blog_id,stock_code,send_time,weibo_url', 'deleted=0', null, 'send_time desc', function(error, data) {
//        if(error) {
//            cb('error', error);
//        } else {
//            var blogids = [];
//            for(var i=0,len=data.length;i<len;i++) {
//                blogids[i] = data[i].micro_blog_id;
//            }
//            mysql._select('micro_blog', 1, param.cnt, 'id,content', 'id IN ('+blogids.join(',')+')', null, null, function(err, res) {
//                if(err) {
//                    cb('error', err);
//                } else {
//                    var blogs = {};
//                    for(var i=0,len=res.length;i<len;i++) {
//                        blogs[res[i].id] = res[i].content;
//                    }
//                    for(var i=0,len=data.length;i<len;i++) {
//                        data[i].content = blogs[data[i].micro_blog_id];
//                    }
//                    cb(null, data);
//                }
//            });
//        }
//    });
    mysql._select('micro_blog', 1, param.cnt, 'stock_code,send_time,content', "content_type IN ('zixun','yd_zhangdie','yd_zhenfu','yd_huanshou')", null, 'send_time desc', function(error, data) {
        if(error) {
            cb('error', error);
        } else {
            cb(null, data);
...
exports.getAccountList = function(cb){
    var condition = "status=0 AND weibo_user_id!=''";
    mysql._select('account', 0, 1, 'weibo_user_id,stock_code', condition, null, null, function(error, data) {
        if(error) {
            cb('error', error);
        } else {
            mysql.getFollowers(function(err, res) {
                if(err) {
                    cb('error', err);
                } else {
                    for(var i=0,len=data.length;i<len;i++) {
                        if(res[data[i].stock_code]) {
                            data[i].followers = res[data[i].stock_code];
                        } else {
                            data[i].followers = 0;
                        }
                    }
...
exports.getHotList = function(param, cb){
    var myDate = new Date();
    var now = myDate.getTime() / 1000;
    now -= 86400;
    var condition = 'in_time > ' + parseInt(now);
    mysql._select('micro_blog', 1, param.cnt, 'stock_code,COUNT(*) as num', condition, 'stock_code', 'num desc', function(error, data) {
        if(error) {
            cb('error', error);
        } else {
            cb(null, data);
