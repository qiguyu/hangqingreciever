...
exports.getLatest = function(param, cb) {
    ...
    mysql._select('sent_micro_blog', 1, cnt, 'micro_blog_id,stock_code,send_time,weibo_url', 'deleted=0', 'send_time desc', function(error, data) {
        if(error) {
            cb('error', error);
        } else {
            var blogids = [];
            ...
            mysql._select('micro_blog', 1, cnt, 'id,content', 'id IN ('+blogids.join(',')+')', null, function(err, res) {
                if(err) {
                    cb('error', err);
                } else {
                    var blogs = {};
...
exports.getAccountList = function(cb){
    ...
    mysql._select('account', 0, 1, 'weibo_user_id,stock_code', condition, null, function(error, data) {
        if(error) {
            cb('error', error);
        } else {
