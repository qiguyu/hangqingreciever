...
exports.formatHqNumbers = function(pObj) {
    ...
    //最高涨跌幅
    var highmarkup = (pObj.high - pObj.close) * 100 / pObj.close;
    hqData.highmarkup = highmarkup.toFixed(2);
    //最低涨跌幅
    var lowmarkup = (pObj.low - pObj.close) * 100 / pObj.close;
    hqData.lowmarkup = lowmarkup.toFixed(2);
    //最高价
    var hqHigh = pObj.high;
    if(typeof hqHigh == 'string') {
        hqHigh = parseFloat(hqHigh);
    }
    hqData.hqHigh = hqHigh.toFixed(2);
    //最低价
    var hqLow = pObj.low;
    if(typeof hqLow == 'string') {
        hqLow = parseFloat(hqLow);
    }
    hqData.hqLow = hqLow.toFixed(2);
    //最高涨跌前缀
    hqData.hqHighPrefix = '';
    if(highmarkup > 0) {
        hqData.hqHighPrefix = '+';
    ...
    //成交量
    var volum = shortnumber(pObj.volum);
    volum += '手';
    hqData.volum = volum;
    //成交额
    var amount = shortnumber(pObj.amount);
    amount += '元';
